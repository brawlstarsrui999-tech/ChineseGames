...
def strip_noise(src: str) -> str:
    """Убираем литералы и комментарии, не принимая ``https://`` за комментарий."""
    # Строки убираем раньше комментариев: иначе ``//`` внутри URL съедает
    # остаток исходника и даёт ложные ошибки для импортов ниже по файлу.
